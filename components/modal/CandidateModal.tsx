...
export default function CandidateModal({
  ...
  return (
    <div className="min-h-full bg-[var(--rb-bg-page)]">
      ...
      <div className="border-b border-[var(--rb-border)] bg-[var(--rb-bg-surface)]">
        <div className="mx-auto max-w-3xl px-6 py-8">
          <div className="flex items-start gap-5">
            {/* Avatar */}
            <div className="size-16 shrink-0 rounded-full bg-[var(--rb-brand)] flex items-center justify-center text-white text-xl font-bold shadow-[0_4px_16px_rgb(79_70_229_/_0.25)]">
              {getInitials(fullName)}
            </div>

            {/* Info */}
            <div className="flex-1 min-w-0">
              <h1 className="text-2xl font-bold text-[var(--rb-text)] leading-tight">{fullName}</h1>
              {targetRole && (
                <p className="text-base text-[var(--rb-text-secondary)] mt-0.5">{targetRole}</p>
              )}
              <div className="flex items-center flex-wrap gap-x-4 gap-y-1 mt-2">
                {location && (
                  <span className="flex items-center gap-1 text-sm text-[var(--rb-text-muted)]">
                ...
                {linkedinUrl && (
                  <a
                    ...
                    className="flex items-center gap-1 text-sm text-[var(--rb-brand)] hover:underline"
                  >
          ...
          {/* Headline */}
          {headline && (
            <p className="mt-5 text-base text-[var(--rb-text-secondary)] leading-relaxed border-l-2 border-[var(--rb-brand)] pl-4">
              {headline}
            </p>
          )}
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 whitespace-nowrap px-3 py-2.5 text-xs font-semibold border-b-2 transition-all duration-[var(--duration-fast)] ${
                      isActive
                        ? 'border-[var(--rb-brand)] text-[var(--rb-brand)]'
                        : 'border-transparent text-[var(--rb-text-muted)] hover:text-[var(--rb-text-secondary)] hover:border-[var(--rb-border-strong)]'
                    }`}
                  >
      ...
      <div className="mx-auto max-w-3xl px-6 py-6">
        {activeTab === null && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <p className="text-[var(--rb-text-muted)] text-sm">No media uploaded yet.</p>
          </div>
        )}
        ...
        {(activeTab === 'audio' || activeTab === 'debate_audio') && (() => {
          ...
          return (
            <div className="rb-card p-6 max-w-md mx-auto">
              <div className="text-center mb-2 text-sm font-semibold text-[var(--rb-text)]">
