...
| Payments | Paddle (employer tiers only — candidates always free) |
...
Candidate tier is always free; Paddle handles employer billing only. Webhook at
`/api/webhooks/paddle` — always verify the signature before processing. Price-id env vars:
`PADDLE_EMPLOYER_{STARTER,GROWTH,SCALE}_PRICE_ID` ($49 / $99 / $249 per mo).
